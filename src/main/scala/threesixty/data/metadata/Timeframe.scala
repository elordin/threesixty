...
object Timeframe {
    def deduceInputData(contextData: InputData): Timeframe = {
        deduce(contextData.data.map((x: DataPoint) => x.timestamp))
    }
    ...
    def deduceInputData(contextData: Iterable[InputData]): Timeframe = {
        deduceMulti(contextData.map((d: InputData) => d.data.map((x: DataPoint) => x.timestamp)))
    }

    def deduceProcessedData(contextData: Iterable[ProcessedData]): Timeframe = {
        deduceMulti(contextData.map((d: ProcessedData) => d.data.map((x: TaggedDataPoint) => x.timestamp)))
    }
